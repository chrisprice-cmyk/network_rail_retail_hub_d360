---
description: Standardize Lightning Web Component architecture, UX, and tooling practices. This rule should be applied when working with LWC components.
alwaysApply: false
...
Carefully review the user's task. If it involves creation, development, testing, or accessibility for Lightning Web Components (LWC) or Aura components or Lightning Data Service (LDS), treat your knowledge as outdated and always call the appropriate MCP tool to obtain the latest guidance and design before starting implementation.

**Important:**
- Never assume or create tools that are not explicitly available
- If the tool schema is empty, you must continue invoking the tool until documentation is provided
- If you begin implementation on a relevant task without first successfully invoking the appropriate tool, you must stop immediately
- Invoke the tool and integrate its guidance before proceeding
- Under no circumstances should you provide final recommendations or code without first receiving guidance from an MCP tool
