...
echo
git --no-pager diff HEAD^ HEAD --name-only --diff-filter=ACM

#push the list of deltas into a text file for debugging purposes
git --no-pager diff HEAD^ HEAD --name-only --diff-filter=ACM > new-modified-files.txt

echo
echo 'Create zip file gitUpdates.zip with new and modified files'
echo
git archive -o gitUpdates.zip HEAD $(git --no-pager diff HEAD^ HEAD --name-only --diff-filter=ACM)
