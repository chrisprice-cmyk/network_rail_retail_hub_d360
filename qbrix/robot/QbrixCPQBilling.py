...
class QbrixCPQBilling(QbrixRobotTask):
    ...
    def go_to_cpq_settings_page(self):
        ...
        self.browser.wait_for_elements_state(f"{self.shared.iframe_handler()} div.sbTitles >> h1:text-is('Salesforce CPQ')", ElementState.visible, "300s")

    def get_cpq_setting_element(self, tab_name, setting_name, setting_element_type = "input"):
    ...
    def set_cpq_setting_select(self, tab_name, setting_label, setting_value):
        ...
        temp_select_setting = self.get_cpq_setting_element(tab_name, setting_label, "select")
        if not self.shared.is_option_selected(temp_select_setting, setting_value):
            self.browser.select_options_by(temp_select_setting, SelectAttribute.label, setting_value)
    ...
    def save_cpq_settings_changes(self):
        ...
        self.browser.click(f"{self.shared.iframe_handler()} div.sbButtons >> input.btn:text-is('Save')")
    ...
    def go_to_billing_settings_page(self):
        ...
        self.browser.wait_for_elements_state(f"{self.shared.iframe_handler()} span.slds-text-heading--label:text-is('Billing Configuration')", ElementState.visible, "300s")
    ...
    def set_demo_configuration_for_billing(self):
        ...
        self.shared.wait_and_click(f"{self.shared.iframe_handler()} ul.slds-tabs--scoped__nav >> li.slds-tabs--scoped__item >> a:text-is('General')")
        if "checked" in self.browser.get_element_states(f"{self.shared.iframe_handler()} table.slds-table:visible >> tbody >> tr.slds-hint-parent:has-text('Disable triggers') >> label.slds-checkbox--toggle >> input[type='checkbox']"):
            self.browser.click(f"{self.shared.iframe_handler()} table.slds-table:visible >> tbody >> tr.slds-hint-parent:has-text('Disable triggers') >> label.slds-checkbox--toggle")

        # Enable Payment > Save Credit Card Details
        self.shared.wait_and_click(f"{self.shared.iframe_handler()} ul.slds-tabs--scoped__nav >> li.slds-tabs--scoped__item >> a:text-is('Payment')")
        sleep(1)
        if "checked" not in self.browser.get_element_states(f"{self.shared.iframe_handler()} table.slds-table:visible >> tbody >> tr.slds-hint-parent:has-text('Save credit card details') >> label.slds-checkbox--toggle >> input[type='checkbox']"):
            self.browser.click(f"{self.shared.iframe_handler()} table.slds-table:visible >> tbody >> tr.slds-hint-parent:has-text('Save credit card details') >> label.slds-checkbox--toggle")

        # Save Changes
        self.browser.click(f"{self.shared.iframe_handler()} div[id='payNow'] >> button:text-is('Save')")
        self.shared.wait_and_click(f"{self.shared.iframe_handler()} button:text-is('Yes')")
