...
class QbrixToolingKeywords(QbrixRobotTask):
    ...
    def enable_admin_auth_for_connected_app(self, connected_app_name, browse_to_app_page: bool = True, waittime:str="15s"):
        """
        ...
        try:
            ...
            if self.browser.get_element_count(f"{iframe_selector} td.dataCol:text-is('Admin approved users are pre-authorized')") == 0:
                ...
                self.browser.wait_for_elements_state(f"{iframe_selector} h2.mainTitle:text-is('Connected App Edit')", ElementState.visible, waittime)
                ...
                if "visible" in self.browser.get_element_states("iframe >>> .btn:has-text('Ok')"):
                    ...
                    sleep(2)
                self.browser.wait_for_elements_state(f"{iframe_selector} .btn:has-text('Save')", ElementState.visible, waittime)
                self.browser.click(f"{iframe_selector} .btn:has-text('Save')")
                self.browser.wait_for_elements_state(f"{iframe_selector} h2.mainTitle:text-is('Connected App Detail')", ElementState.visible, waittime)
        except Exception as e:
            self.browser.take_screenshot()
            raise e

    def enable_system_admin_profile_for_connected_app(self, connected_app_name: str, browse_to_app_page: bool = True, waittime:str="15s"):
        """
        ...
        try:
            ...
            iframe_selector = self.shared.iframe_handler()
            self.browser.wait_for_elements_state(f"{iframe_selector} .btn:has-text('Manage Profiles')", ElementState.visible, waittime)
            self.browser.click(f"{iframe_selector} .btn:has-text('Manage Profiles')")
            self.browser.wait_for_elements_state(f"{iframe_selector} h1:text-is('Application Profile Assignment')", ElementState.visible, waittime)
    ...
    def enable_vbt_passport(self):
        ...
        self.enable_system_admin_profile_for_connected_app(connected_app_name, False,"40s")
