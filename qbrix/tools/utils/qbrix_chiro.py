...
class OmniscriptAlign(Command):
    ...
    def createworkingarea(self, username:str):
        ...
        subprocess.run([f"sfdx force:project:create --projectname {hashname} --json"], shell=True, capture_output=True,cwd=".qbrix")
        
        subprocess.run([f"sfdx force:config:set defaultusername={username} --json"], shell=True, capture_output=True,
                    cwd=qbrixtempdir)
        
        subprocess.run([f"sfdx force:config:set instanceUrl={self.instanceurl} --json"], shell=True, capture_output=True,
                cwd=qbrixtempdir)
    ...
    def retrievelwcs(self,username:str):
        ...
        try:
            ...
            subprocess.run([f"export SFDX_ACCESS_TOKEN='{self.accesstoken}' && sfdx force:auth:accesstoken:store --instanceurl {self.instanceurl} -a {hashname} --noprompt --json --loglevel DEBUG  && sfdx force:source:retrieve -u {hashname} -m LightningComponentBundle"], shell=True, capture_output=True,cwd=qbrixtempdir)
    ...
    def pushlwcs(self,username:str):
        ...
        try:
            ...
            #now we need to inject a sfdx session and into the cci runtimee for that temp dir
            subprocess.run([f"sfdx force:source:deploy -u {hashname} -m LightningComponentBundle"], shell=True, capture_output=True,cwd=qbrixtempdir)
    ...
    def getoslist(self, username:str, namespaceprefix:str):
        ...
        for token in jsondata["result"]["records"]:
            if namespaceprefix != "omnistudio":
                ...
                try:
                    objectdata = OmniScript(token[f"{namespaceprefix}__Type__c"], token[f"{namespaceprefix}__SubType__c"],
