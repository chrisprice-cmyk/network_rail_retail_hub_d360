...
class ExperienceManager(BaseSalesforceApiTask):
    ...
    task_options = {
        ...
        "experience_site_name": {"description": "Name/label for the Experience Cloud site", "required": True},
        "featured_topics": {"description": "Dictionary of featured topics with related image file path", "required": False},
        "topic_assignments": {"description": "Dictionary of topics with a list of related knowledge article titles", "required": False},
        "collaboration_groups": {"description": "List of collaboration groups to link to site", "required": False},
    }
    ...
    def _init_options(self, kwargs):
        ...
        self.collaboration_groups = self.options["collaboration_groups"] if "collaboration_groups" in self.options else None
    ...
    def _process_collaboration_groups(self):
        ...
        for collaboration_group in self.collaboration_groups:
            ...
            cg_lookup = self.sf.query(f"select id from CollaborationGroup Where Name = '{collaboration_group}' and NetworkId = '{self.network_id}' LIMIT 1")
            ...
            if cg_lookup["totalSize"] == 0:
                ...
                if group_id and group_id['id']:
                    self.logger.info(f"Created new Collaboration Group '{collaboration_group}'  for experience cloud site '{self.experience_site_name}' with id '{group_id['id']}'")
            else:
                self.logger.info(f"Collaboration Group '{collaboration_group}' was already created")

    def _lookup_topic_id(self, topic_name):

        topic_lookup = self.sf.query(f"select id,Name from Topic Where Name = '{topic_name}' LIMIT 1")
    ...
    def _lookup_article_id(self, article_title):
        ...
        knowledge_lookup = self.sf.query(f"Select id from Knowledge__kav Where Title =  '{article_title}' LIMIT 1")
    ...
    def _process_topic_assignments(self):
        ...
        for topic, kb_articles in self.topic_assignments.items():
            ...
            if not topic_id:
                self.logger.info(f"Topic with name '{topic_name}' not found. Skipping...")
                continue
            ...
            for article in kb_articles:
                ...
                if not knowledge_article_id:
                    self.logger.info(f"Knowledge Article with title '{knowledge_title}' not found. Skipping...")
                    continue

                kb_lookup = self.sf.query(f"select id from TopicAssignment Where TopicId = '{topic_id}' and NetworkId = '{self.network_id}' and EntityId = '{knowledge_article_id}' LIMIT 1")
                ...
                if kb_lookup["totalSize"] == 0:
                    ...
                    if topic_assignment_id and topic_assignment_id.get('id'):
                        self.logger.info(f"Created new Topic Assignment for experience cloud site {self.experience_site_name} with id {topic_assignment_id['id']}")
                    else:
                        self.logger.error("Topic failed to create:")
                        self.logger.info(topic_assignment_id)

                else:
                    self.logger.info(f"Topic Assignment already created for article '{article}' and topic '{topic}' ")
    ...
    def _process_featured_topic(self):
        ...
        with open(".qbrix/feature_topic_uploads/temp_robot_topics_upload.robot", 'w', encoding="utf-8") as upload_file:
            upload_file.write(robot_file)

        run_command(f"cci task run robot --org {self.org_config.name} --suites .qbrix/feature_topic_uploads/temp_robot_topics_upload.robot --vars 'browser:headlesschrome'")
    ...
    def _run_task(self):
        ...
        site_lookup = self.sf.query(f"SELECT Id,Name From Network WHERE Name = '{self.experience_site_name}' LIMIT 1")
