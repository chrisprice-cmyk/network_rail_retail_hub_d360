...
class Spin(SFDXBaseTask):
    ...
    def _createworkingarea(self):
        ...
        subprocess.run([f"sfdx force:project:create --projectname {self.devhubuser} --json"], shell=True, capture_output=True,cwd=".qbrix")
        subprocess.run([f"sfdx force:config:set defaultusername={self.devhubuser} --json"], shell=True, capture_output=True,
    ...
    def _deployqbrix(self):
        for(x) in self.deployqbrix:
            ...
            if os.path.isdir(targetdir):
                ...
                with subprocess.Popen(['cci','flow','run','deploy_qbrix','--org',self.cciorg], stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, universal_newlines=True,cwd=targetdir) as p:
    ...
    def _monitorrequest(self):
        ...
        if(hasattr(self,"signuprequestid")):
            while(self._checktempaltestatuscomplete()==False):
                sleep(60)
                maxwait=maxwait-1
    ...
    def _checktempaltestatuscomplete(self):
        ...
        if jsonresult["status"] == 0:
            ...
            if(jsonresult["result"]["Status"] == "Success" ):
                ...
                if(not self.devhubconsumerkey is None  and not self.devhubjwtkeyfile is None):
                    ...
                    while(spinjwtresult["status"]==1):
                        ...
                        if(maxjwt==0):
                            raise CommandException("Unable to establish JWT authentication to template spin within poll time")

                        
                    self.logger.info(spinjwtresult)
                    self.jwtresult=spinjwtresult
                    if(not spinjwtresult is None ):
